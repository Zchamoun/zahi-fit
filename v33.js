"use strict";
/* Zahi Fit v3.3.0 — Professional Visual Asset Library */
(() => {
  const VERSION = "v3.3.0";
  const RATE_KEY = "zahiFitVoiceRateV320";
  const MODE_KEY = "zahiFitVoiceModeV320";
  const VOICE_KEY = "zahiFitVoiceNameV320";
  ...
  function photoAssetFor(family, idx){
    // v3.3: dedicated five-step visual asset for every mapped exercise family.
    // Keep the approved photographic World's Greatest Stretch sequence.
    if(family==="worldStretch") return `./pt-assets/world-stretch-step${idx+1}.jpg`;
    return `./pt-assets-v33/${family}-step${idx+1}.svg`;
  }
  ...
  function demoVisualHtml(ex, idx, thumb=false){
    ...
    if(thumb){
      return `<img class="zf31-photo zf31-photo-thumb" src="${asset}" alt="${esc(ex.n)} — ${esc(step.title)}">`;
    }
    return `<div class="zf31-photo-wrap zf33-pro-visual">
